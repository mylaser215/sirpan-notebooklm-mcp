...
### Fixed
- **`detect_drift` ambiguous on duplicate filenames across folders (v7)** — NLM-stored source titles were bare filenames (e.g. `SKILL.md`, `nlm_seed.md`) so the multi-layer matcher in `sync_helpers.find_disk_path` could not pick a single disk path among 35/3 candidates. Two coordinated changes: (1) `services/sources.add_source` now stamps `{basename} ({parent_folder})` titles automatically when `add_file` is called without an explicit title for a name in `DUP_FILENAMES_AUTO_FOLDER_TAG = {"SKILL.md", "nlm_seed.md"}`, and (2) `services/sync_helpers._narrow_nlm_seed` was generalized to `_narrow_by_folder_hint` which recognizes the new `{filename} ({folder})` pattern across all files while preserving the legacy Korean keyword fallback. Pre-existing residual sources need a one-shot `rename_source` migration since NLM has no folder metadata. Regression: `tests/test_sync_helpers.py` (+4 cases — folder_hint matches/no_match/no_hint compat/v7 real residual).
