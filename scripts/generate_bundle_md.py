...
def generate_bundle(
    files: list[Path], output: Path, bundle_name: str, root: Path | None = None
) -> None:
    """Build bundle in a temp dir then atomically copy to `output`.

    On any failure the temp dir is removed and `output` is not touched.
    """
    _validate_inputs(files)
    timestamp = datetime.now().strftime("%Y-%m-%dT%H:%M")
    rendered = _render_bundle(files, bundle_name, timestamp, root)
...
def main() -> int:
    ...
    try:
        ...
        generate_bundle(files, args.output.resolve(), args.bundle_name, root=root)
