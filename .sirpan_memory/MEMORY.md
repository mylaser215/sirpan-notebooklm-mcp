...
## 현재 상태 (260514-세션37·v6 졸업·source_add 진단)
...
- ✅ **세션37 source_add 진단** (260514, commit `67a3eb7`): state-level 결함(가설 d, NLM 측 dedup/partial-state) + Source-Note 라우팅 도메인 결함 발견. `error_result`에 `detail` 필드 8 wrappers 노출 — 재발 시 진단 보험
...
## 최근 세션
- 세션37 (260514): source_add state-level 진단 + Source-Note 라우팅 결함 발견. `detail` 필드 8 wrappers 보강 (commit `67a3eb7`). v7 시드 todo 신설
