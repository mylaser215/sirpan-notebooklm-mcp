"""세션58 ATOM-2 (upstream eadb8c3 #212): regression tests for the auth/studio
silent-failure bug.
...
These tests pin the *desired* contract. They are RED against upstream 0.6.13 and
GREEN after the fix. All auth states are mocked — no network, no real credentials.

우리 fork 호환: `_is_rts_expiring` 우리 fork RTS 가드(`bbae413`) 분기를 *healthy*로
강제 monkeypatch — upstream #212가 다루는 *disk reload 후 live 검증* 경로 시험을
위해서 RTS healthy 전제 필요.
"""
...
def test_refresh_auth_does_not_claim_success_when_tokens_are_expired(monkeypatch):
    """refresh_auth() must NOT return status:"success" while the reloaded tokens
    are actually unusable. A disk reload of dead tokens is not a successful re-auth.

    우리 fork RTS 가드를 우회하여 *disk reload 후 live 검증* 경로 진입을 보장.
    """
    ...
    monkeypatch.setattr(
        core_auth, "_is_rts_expiring", lambda *a, **kw: False, raising=True
    )
    monkeypatch.setattr(
        core_auth,
        "load_cached_tokens",
    ...
    # Live check reports tokens expired.
    monkeypatch.setattr(
...
def test_refresh_auth_reports_success_when_tokens_are_valid(monkeypatch):
    """Happy path: valid tokens on disk + live check passes → success."""
    monkeypatch.setattr(auth_tools, "get_client", lambda: _FakeClient(), raising=True)
    monkeypatch.setattr(auth_tools, "reset_client", lambda: None, raising=True)
    monkeypatch.setattr(
        core_auth, "_is_rts_expiring", lambda *a, **kw: False, raising=True
...
def test_studio_create_fails_loudly_on_stale_auth(monkeypatch):
    """When auth is stale/expired, studio_create() must return status:"error"
    BEFORE firing a doomed generation request — not status:"success" with an
    artifact_id that immediately fails."""
    monkeypatch.setattr(
...
def test_studio_status_synthesizes_reason_for_failed_artifact_without_raw_error(monkeypatch):
    """The real gRPC payload gives failed artifacts NO error string. The service
    must still surface a non-null error_reason so an agent stops polling and acts."""
    failed_artifact = {
...
def test_studio_create_e2e_per_auth_state(monkeypatch, auth_state, valid, reason):
    """Each auth state must yield a deterministic, actionable outcome."""
    monkeypatch.setattr(
