...
class ConversationMixin(BaseClient):
    ...
    def _build_conversation_history(
        ...
        with self._state_lock:
            if conversation_id in self._conversation_cache:
                # 세션58 ATOM-1 (upstream 46fe5a0 #213): mark as recently used
                # for LRU eviction.
    ...
    def _cache_conversation_turn(self, conversation_id: str, query: str, answer: str) -> None:
        ...
        세션58 ATOM-1 (upstream 46fe5a0 #213): The cache is bounded so long-lived
        MCP server processes don't grow without bound:
        - Each conversation keeps only the last `_max_turns_per_conversation`
          turns (FIFO; oldest dropped first). Turn numbers are renumbered 1..N
          after trim so `turn_number` stays a stable 1-indexed position.
        - The whole dict is LRU-bounded to `_max_conversations`. On overflow
          the least-recently-used conversation is evicted.
        - Each turn's answer is truncated to `_max_chars_per_turn` chars as a
          safety net. Queries are user input and never truncated.

        Set any knob to 0 (env var) to disable that specific cap.
        """
        with self._state_lock:
            # Per-turn answer truncation (safety net; queries are user input
            # and not truncated).
            ...
            # survivors so `turn_number` always means "1-indexed position in
            # the current list" (matches the original unbounded-cache semantics
    ...
    def get_conversation_history(self, conversation_id: str) -> list[dict[str, str | int]] | None:
        ...
        with self._state_lock:
            if conversation_id in self._conversation_cache:
                # 세션58 ATOM-1 (upstream 46fe5a0 #213): LRU promotion on read.
                self._conversation_cache.move_to_end(conversation_id)
                turns = list(self._conversation_cache[conversation_id])
            else:
                turns = []
    ...
    def get_conversation_cache_stats(self) -> dict[str, int]:
        ...
        세션58 ATOM-1 (upstream 46fe5a0 #213): Useful for debugging, tests,
        and exposing memory pressure via tooling.
    ...
    def query(
        ...
        if server_conv_id and server_conv_id != conversation_id:
            with self._state_lock:
                # Migrate local cache to the server-assigned ID. 세션58 ATOM-1
                # (upstream 46fe5a0 #213): use move_to_end unconditionally so
                # the migrated key is at MRU even if server_conv_id was already
                # in the cache (the bare assignment wouldn't reorder it).
                if conversation_id in self._conversation_cache:
